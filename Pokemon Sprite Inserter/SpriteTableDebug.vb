...
Public Class Sprite_Table_Debug
    ...
    Public Sub DrawSprite(ByVal SpriteData As String, ByVal SpritePaletteData As String, ByVal SpriteSize As Size, ByVal SpriteControl As Control,
        ...
        AddHandler SpritePictureBox.MouseClick, AddressOf PanelDeselect
    ...
    Private Sub LoadData()
        PatchFile = SpritePatchFilePathTextBox.Text
        If CInt(GlobalPaletteNumberTextBox.Text) < PaletteDataArray.Length Then
            CurrentPaletteIndex = CInt(GlobalPaletteNumberTextBox.Text)
        End If
        SelectionCountTextBox.Text = "0"
        FlowLayoutPanel1.Controls.Clear()
        Dim SpriteFrameDataStartOffset As String = "3A00A0"
        Dim Max As Integer = CInt(CountTextBox.Text)
        Dim SpriteCount As Integer = If(CInt(StartFromTextBox.Text) > 0, CInt(StartFromTextBox.Text) - 1, 0)
    ...
    Public Sub PanelSelect(sender As Object, e As MouseEventArgs)
        If e.Button = MouseButtons.Left Then
            ...
            If SelectPanel.Parent().BackColor <> Color.LightSteelBlue Then
                ...
                SelectionCountTextBox.Text = CStr(CInt(SelectionCountTextBox.Text) + 1)
            End If
        End If
    End Sub

    Public Sub PanelDeselect(sender As Object, e As MouseEventArgs)
        If e.Button = MouseButtons.Right Then
            Dim SelectPanel As PictureBox = DirectCast(sender, PictureBox)
            If SelectPanel.Parent().BackColor = Color.LightSteelBlue Then
                SelectPanel.Parent().BackColor = SelectPanel.Parent().Tag
                If CInt(SelectionCountTextBox.Text) > 0 Then
                    SelectionCountTextBox.Text = CStr(CInt(SelectionCountTextBox.Text) - 1)
                End If
            End If
        End If
    ...
    Private Sub LoadSpritesButtonClick(sender As Object, e As EventArgs) Handles LoadSpritesButton.Click
        LoadData()
    End Sub

    Private Sub SpriteTableDebugLoad(sender As Object, e As EventArgs) Handles MyBase.Load
        AddHandler StartFromTextBox.KeyPress, AddressOf DigitValidator
        AddHandler StartFromTextBox.KeyPress, AddressOf NonZeroValidator
        AddHandler StartFromTextBox.KeyPress, AddressOf MaxLimitValidator
        AddHandler CountTextBox.KeyPress, AddressOf DigitValidator
        AddHandler CountTextBox.KeyPress, AddressOf NonZeroValidator
        AddHandler CountTextBox.KeyPress, AddressOf MaxLimitValidator
        AddHandler GlobalPaletteNumberTextBox.KeyPress, AddressOf DigitValidator
        AddHandler GlobalPaletteNumberTextBox.KeyPress, AddressOf DigitValidator
        AddHandler GlobalPaletteNumberTextBox.KeyPress, AddressOf MaxLimitValidator
        RomTextBox.Text = Main.RomFilePath
    End Sub

    Private Sub DeselectButtonClick(sender As Object, e As EventArgs) Handles DeselectButton.Click
    ...
    Private Sub OpenSpritePatchFileButtonClick(sender As Object, e As EventArgs) Handles OpenSpritePatchFileButton.Click
        SpritePatchSaveFileDialog.Filter = "Text Files (*.txt*)|*.txt"
        SpritePatchSaveFileDialog.Title = "Load Patch File"
        Try
            If SpritePatchSaveFileDialog.ShowDialog <> Windows.Forms.DialogResult.Cancel Then
                PatchFile = SpritePatchSaveFileDialog.FileName
                SpritePatchFilePathTextBox.Text = PatchFile
                LoadSpritesButton.Enabled = True
            End If
        Catch ex As Exception
            LoadSpritesButton.Enabled = False
            MessageBox.Show("An error occurred while opening the patch file!" & vbCrLf & vbCrLf & "Exception.Message : " + ex.Message, "Error In Opening Rom!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub

    Private Sub HowToUseButtonClick(sender As Object, e As EventArgs) Handles HowToUseButton.Click
